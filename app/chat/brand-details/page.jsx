...
export default function BrandDetailsPage() {
  ...
  const previews = useMemo(() => {
    ...
    const primaryGoal = (formData?.primaryGoal || "").trim() || "Add your primary goal";
    const goalsArr = Array.isArray(formData?.goals) ? formData.goals : [];
    const goalsPreview =
      goalsArr.length > 0
        ? `${primaryGoal} • ${goalsArr.slice(0, 2).join(", ")}${goalsArr.length > 2 ? "…" : ""}`
        : primaryGoal;

    const servicesArr = Array.isArray(formData?.services) ? formData.services : [];
    const servicesPreview =
      servicesArr.length > 0
        ? `${servicesArr.length} services: ${servicesArr.slice(0, 2).join(", ")}${servicesArr.length > 2 ? "…" : ""}`
        : "Add the services you offer";

    const diffArr = Array.isArray(formData?.differentiators) ? formData.differentiators : [];
    const diffPreview =
      diffArr.length > 0 ? `${diffArr[0]}${diffArr.length > 1 ? "…" : ""}` : "Add what makes you different";

    const voiceArr = Array.isArray(formData?.values) ? formData.values : [];
    const voicePreview =
      voiceArr.length > 0 ? `Voice: ${voiceArr.slice(0, 3).join(", ")}${voiceArr.length > 3 ? "…" : ""}` : "Add brand voice traits";

    return { infoPreview, missionPreview, goalsPreview, servicesPreview, diffPreview, voicePreview };
  }, [formData]);
  ...
  async function handleSave() {
    ...
    // front-end only: save to localStorage for now
    try {
  ...
  function handleConfirmFinal() {
    // front-end only: set a flag you can later replace with DB state
    try {
